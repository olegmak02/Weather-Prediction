...
import numpy as np
import pandas as pd
from keras.src.layers import Conv1D, MaxPooling1D, Flatten
...
if not os.path.exists(model_path):
    model = Sequential()
    model.add(Conv1D(32, 2, activation='relu', input_shape=(window_size, 1)))
...
def nn_rolling_predictions(data):
    ...
    for start in range(len(data)-window_size):
        ...
        pred = scaler_y.inverse_transform([[pred[0,0]]])[0, 0]
